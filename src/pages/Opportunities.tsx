...
const Opportunities = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['id', 'customerName', 'description', 'estimateRevenue', 'stage', 'type', 'isUrgent', 'estimateClose']);
  ...
  const getProductData = useCallback((opportunity: Opportunity) => {
    const primaryProduct = opportunity.productGroups.flatMap(group => group.products).find(product => product.isPrimary);
    return {
  ...
  const getFieldValue = useCallback((opportunity: Opportunity, field: string) => {
    ...
    switch (field) {
      case 'stage':
        ...
        return stage ? stage.id.toString() : '';
      // Return ID as string for sorting
      case 'type':
  ...
  const stageData = useMemo(() => {
    ...
    const filteredStages = stageCounts.filter(stage => stage.count > 0).sort((a, b) => a.id - b.id);
  ...
  const selectAll = () => {
    setSelectedIds(current => current.length === paginatedOpportunities.length ? [] : paginatedOpportunities.map(opp => opp.id));
  };
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      return <div className="bg-white p-2 border border-gray-300 rounded shadow">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {stageFilter && <div className="mt-4">
        ...
        {/* Filters */}
        <OpportunityFilters filters={filters} onUpdateFilters={setFilters} showClosedOpportunities={showClosedOpportunities} onToggleClosedOpportunities={setShowClosedOpportunities} />

        {/* Table Controls */}
        <div className="flex justify-between items-center">
          <div className="flex gap-2">
            <ColumnCustomizer visibleColumns={visibleColumns} onUpdateColumns={setVisibleColumns} />
            <Button variant={editMode ? "default" : "outline"} onClick={() => setEditMode(!editMode)}>
              <Edit className="w-4 h-4 mr-2" />
              {editMode ? 'Exit Edit Mode' : 'Edit Mode'}
            </Button>
            {selectedIds.length > 0 && <BulkEditDialog selectedCount={selectedIds.length} onBulkEdit={handleBulkEdit} />}
          </div>
        ...
        <Card>
          <CardContent className="p-0">
            <OpportunityTable opportunities={paginatedOpportunities} visibleColumns={visibleColumns} selectedIds={selectedIds} editMode={editMode} sortConfig={sortConfig} onSort={handleSort} onToggleSelection={toggleSelection} onSelectAll={selectAll} onUpdateOpportunity={(id, updates) => {
            setOpportunities(current => current.map(opp => opp.id === id ? {
              ...opp,
              ...updates
            } : opp));
          }} />
        ...
        {/* Pagination */}
        {pageSize !== -1 && totalPages > 1 && <div className="flex justify-center">
            <Pagination>
              <PaginationContent>
                <PaginationItem>
